<?php
	...
	list($site_api, $plugins, $categories) = Koken::api( array(
			'/site' . ( $draft ? ( $preview ? '/preview:' . $preview : '/draft:true') : '' ),
			'/plugins',
			'/categories/summary:false'
		)
	);
	...
	else
	{
		...
		foreach($routes as $route => $page)
		{
			...
			if (preg_match('~^' . $route . '$~u', $url, $matches))
			{
				...
				Koken::$page_class = trim(Koken::$page_class . ' k-lens-' . $page['template']);
