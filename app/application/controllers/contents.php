...
class Contents extends Koken_Controller {
	...
	function index()
	{
		...
		if ($slug || (isset($id) && strpos($id, ',') === false))
		{
			...
			if ($options['context'] && $options['context'] !== 'stream' && $options['context'] !== 'favorites' && strpos($options['context'], 'tag-') === false && strpos($options['context'], 'category-') === false)
			{
			...
			if ($content->exists())
			{
				...
				if ($options['context'] && $original_context !== 'stream' && $original_context !== 'favorites' && strpos($original_context, 'tag-') === false && strpos($original_context, 'category-') === false)
				{
					$options['in_album'] = $a;
				}
				...
				if ($options['context'])
				{
					...
					if ($original_context !== 'stream' && $original_context !== 'favorites' && strpos($original_context, 'tag-') === false && strpos($original_context, 'category-') === false)
					{
					...
					else
					{
						...
						{
							$tag = str_replace('tag-', '', $original_context);
							$t = new Tag;
							$t->slug = $tag;
